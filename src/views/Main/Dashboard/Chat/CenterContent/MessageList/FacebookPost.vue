    ...
    <div v-if="get_post_success && !loading" class="w-full sm:h-[180px] h-[190px] my-3 flex justify-end">
        <div class="w-[95%] bg-white rounded-lg p-3">
            <div class="flex text-xs justify-between items-center text-slate-600">  
                <p class="md:w-full w-[50%]">{{ $t('v1.view.main.dashboard.chat.post.post_by') }}
                    <span class="font-bold" v-if="post_info?.admin_creator?.name">
                        {{ post_info?.admin_creator?.name }}
                    </span> 
                    <span 
                        class="font-bold" 
                        v-if="!post_info?.admin_creator?.name && post_info?.from?.name"
                    >
                        {{ post_info?.from?.name }}
                    </span> 
                    <span v-if="post_info?.updated_time">
                        - {{ format(new Date(post_info?.updated_time as string), `dd/MM/yy HH:mm`) }}
                    </span>
                </p>
                <div 
                    class=" rounded-3xl p-1 bg-orange-200 text-orange-500 font-bold whitespace-nowrap cursor-pointer"
                    @click="copyToClipboard(post_info?.id as string, $t('v1.view.main.dashboard.chat.post.copy_to_clipboard'))"
                >
                    <p class="flex">
                        <p class="w-[80px] truncate">{{ post_info?.id }}</p>
                        <span>(Copy)</span>
                    </p>
                </div>
            </div>
            <div class="flex items-center mt-2">
                ...
                >
                    <img
                        src="https://botbanhang.vn/chat/img/logo-bot.e0e6089b.svg" alt=""
                        class="w-[60px] h-[60px]"
                    >
                </object>
            ...
            <div class="flex justify-end mt-2">
                <p 
                    class="text-slate-700 text-xs underline cursor-pointer"
