<template>
    <div v-tooltip.bottom="getLabelInfo(page_id, label_id)?.title" :tooltip-disabled="getLabelConfig() === 'ICON'"
        v-if="getLabelInfo(page_id, label_id)" :style="{ background: getLabelInfo(page_id, label_id)?.bg_color }" :class="{
            'w-[15px] h-[15px]': getLabelConfig()?.includes('ICON')
