            ...
            <div class="flex items-center justify-between mb-2 md:absolute md:top-[-40px] md:right-[9px]">
                ...
                <Modal ref="create_new_website_ref">
                    ...
                    <template v-slot:body>
                        <div
                            class="grid grid-cols-1 md:grid-cols-2 gap-2 max-h-[calc(100vh_-_254px)] overflow-hidden scrollbar-vertical overflow-y-auto">
                            <InputLabel v-model="create_new_website_data.description"
                                :title="$t('v1.view.main.dashboard.select_platform.create_website.description')"
                                :placeholder="$t('v1.view.main.dashboard.select_platform.create_website.enter_description')" />
                            <InputLabel v-model="create_new_website_data.name"
                                :title="$t('v1.view.main.dashboard.select_platform.create_website.link_website')"
                                :placeholder="$t('v1.view.main.dashboard.select_platform.create_website.enter_link_website')" />
                            <InputLabel v-model="create_new_website_data.avatar"
                                :title="$t('v1.view.main.dashboard.select_platform.create_website.link_icon')"
                                :placeholder="$t('v1.view.main.dashboard.select_platform.create_website.enter_link_icon')"
                                class="grid-" />
                        </div>
                    </template>
                    <template v-slot:footer>
                        <div>
                            <button @click="createNewWebsite"
                ...
                <button @click="embeddedSignUpWhatsapp" v-if="current_selected_tab === 'FB_WHATSAPP'"
                    class="w-[130px] h-[40px] text-white bg-green-600 rounded hover:bg-green-700 flex items-center justify-center">
                    <img src="@/assets/icons/whatsapp.svg" width="30" height="30">
                    {{ $t('v1.view.main.dashboard.select_platform.add_page') }}
                </button>
            </div>
            <div v-if="is_loading_page_list" class="absolute left-[50%] translate-x-[-50%]">
                <Loading class="mx-auto" />
            </div>
            <div v-if="!filter_page_list || !filter_page_list.length"
            ...
            <div :class="{ 'md:grid-cols-3 xl:grid-cols-4': commonStore.dashboard_toggle_nav }"
                ...
                <PageItem v-for="page of filter_page_list" :page_info="page.page" @click="toggleSelectThisPage(page)">
                    <div class="flex justify-end">
                        <input :disabled="page.page?.is_active" :checked="page.page?.is_active || page.is_select"
                            type="checkbox" class="w-[15px] h-[15px] mr-2 accent-orange-600">
                    </div>
                </PageItem>
            </div>
        </div>
        <FooterButton :text="$t('v1.view.main.dashboard.select_platform.active')" @click_btn="activePageSelect"
            :disabled="!countSelectPage()" :count="countSelectPage()" />
    </div>
</template>
...
watch(() => current_selected_tab.value, () => filterListPage())
// nạp lại danh sách page thì có thay đổi
watch(() => page_list.value, () => filterListPage())
...
const delayFilterListPage = debounce(
    () => filterListPage(),
    300
)
...
function createNewWebsite() {
    flow([
        // * tạo mới page website
        (cb: CbError) => create_website_page(
            create_new_website_data.value,
            (e, r) => {
                if (e) return cb(e)
        ...
        (cb: CbError) => {
            ...
            toast('success', $t('v1.view.main.dashboard.select_platform.create_website.success'))
...
function activePageSelect() {
    ...
    flow([
        ...
        (cb: CbError) => {
            DATA.list_active_page = filter_page_list.value?.filter(n => n.is_select) || []
        ...
        (cb: CbError) => {
            eachOfLimit(
                ...
                1,
                (page: ThisPageData, i, next) => update_page({
