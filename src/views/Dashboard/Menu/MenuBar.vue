<template>
    ...
    <NavItem v-if="size(pageStore.selected_page_id_list)" :is_active="$route.path.indexOf('/dashboard/download') === 0"
        @click="preGoToChat(() => selectNav($router, '/dashboard/download'))" icon_class="w-[20px]" :icon="downloadSvg"
    ...
    <NavItem :is_active="$route.path.indexOf('/dashboard/noti') === 0"
        @click="selectNav($router, '/dashboard/noti')" :icon="bellSvg"
        :title="$t('v1.view.main.dashboard.nav.noti')" />
    <UserItem :is_active="$route.path.indexOf('/dashboard/user') === 0"
...
const LIST_NAV = [
    {
        path: '/dashboard/select-page',
        icon: pageSvg,
        icon_class: 'w-[20px]',
        title: $t('v1.view.main.dashboard.nav.select_page')
    },
    {
        path: '/dashboard/select-platform',
        icon: linkSvg,
        icon_class: 'w-[20px]',
